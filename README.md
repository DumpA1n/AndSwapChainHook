...
```bash
./AndKittyInjector --package <包名> -lib libAndSwapChainHook.so -dl_memfd -hide_maps -hide_solist -watch -delay 50
```
...
```bash
./AndKittyInjector --package <package_name> -lib libAndSwapChainHook.so -dl_memfd -hide_maps -hide_solist -watch -delay 50
```
